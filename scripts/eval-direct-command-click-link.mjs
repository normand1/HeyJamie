...
/**
 * End-to-end integration test for "Hey Jamie, click the first link" direct commands.
 *
 * Validates that when an initial BrowserOS search is running and the user says
 * "Hey Jamie, click the first link", the direct command executes and the browser
 * URL changes twice (search results page, then the clicked link).
 *
 * Usage:
 *   node scripts/eval-direct-command-click-link.mjs [--output <report.json>] [--timeout <ms>]
 *
 * Environment:
...
const TRANSCRIPT_ID = "direct-command-click-link";
