...
/**
 * End-to-end integration test for the audio pipeline diagram creation feature.
 *
 * Launches the Tauri app with the audio-pipeline-diagram mock transcript,
 * waits for the run-complete event in the integration test log, kills the app,
 * and evaluates the log against expectations.
 *
 * Usage:
 *   node scripts/eval-audio-pipeline-diagram.mjs [--output <report.json>] [--timeout <ms>]
 *
 * Environment:
...
const TRANSCRIPT_ID = "audio-pipeline-diagram";
const VITE_PORT = 1437;
...
async function main() {
  ...
  const child = spawn("npx", ["tauri", "dev", "--config", tauriConfig], {
    ...
    env: {
      ...
      VITE_HEYJAMIE_LLM_MODEL: process.env.HEYJAMIE_LLM_MODEL || "anthropic/claude-sonnet-4.5",
      HEYJAMIE_LLM_MODEL: process.env.HEYJAMIE_LLM_MODEL || "anthropic/claude-sonnet-4.5",
      HEYJAMIE_EXCALIDRAW_TIMEOUT_MS: process.env.HEYJAMIE_EXCALIDRAW_TIMEOUT_MS || "240000",
